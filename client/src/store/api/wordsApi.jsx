...
const wordsApi = createApi({
  reducerPath: 'wordsApi', // Api的标识，不能和其他的Api或reducer重复
  ...
  tagTypes: ['word'],      // 用来指定Api中的标签类型
  endpoints(build) {
    // build是请求的构建器，通过build来设置请求的相关信息
    return {
      getWords: build.query({
        query() {
          return { url:'words'  }  // 用来指定请求子路径
        },
        // transformResponse 用来转换响应数据的格式
        transformResponse(baseQueryReturnValue, meta, arg) {
            return baseQueryReturnValue.data;
        },
        providesTags: [{type: 'word', id: 'LIST'}]
      }),
      getWordById: build.query({
        query(id) {
    ...
    delWord: build.mutation({
      query(id) {
          //http://localhost:1337/api/words/4
          return {
  ...
  updateWord: build.mutation({
      ...
      invalidatesTags: ((result, error, stu) =>
          [{type: 'words', id: stu.id}, {type: 'words', id: 'LIST'}])
  }),
